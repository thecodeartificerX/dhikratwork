...
.DESCRIPTION
    ...
    The script:
      1. Validates prerequisites (.pfx, .cer, MSIX_CERT_PASSWORD, gh CLI)
      ...
      5. Runs flutter pub run msix:create --certificate-password <env var>
    ...
    Prerequisites:
      - windows\signing\CERTIFICATE.pfx must exist (generated by generate-cert.ps1)
      - DhikrAtWork.cer must exist at the repo root (committed)
      - $env:MSIX_CERT_PASSWORD must be set to the .pfx password
...
.EXAMPLE
    .\release.ps1 -Version 0.2.0 -Changelog @"
...
if (-not (Test-Path $PfxPath)) {
    Write-Fail "Certificate not found: $PfxPath"
    Write-Host ''
    Write-Host '  Run .\generate-cert.ps1 to create the signing certificate.' -ForegroundColor Yellow
    exit 1
}
...
if (-not (Test-Path $CerPath)) {
    ...
    Write-Host ''
    Write-Host '  Run .\generate-cert.ps1 to create and export DhikrAtWork.cer.' -ForegroundColor Yellow
    exit 1
}
Write-Pass "DhikrAtWork.cer found"

# MSIX_CERT_PASSWORD environment variable
if ([string]::IsNullOrEmpty($env:MSIX_CERT_PASSWORD)) {
    Write-Fail 'Environment variable MSIX_CERT_PASSWORD is not set.'
    Write-Host ''
    Write-Host '  Set it before running this script:' -ForegroundColor Yellow
    Write-Host '    $env:MSIX_CERT_PASSWORD = "your-cert-password"' -ForegroundColor Yellow
    exit 1
}
...
& git -C $ProjectRoot commit -m "chore: bump version to ${Version}"
